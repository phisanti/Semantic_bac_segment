...
class MultiClassWeightedBinaryCrossEntropy(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        for channel in range(num_channels):
            ...
            # Calculate the binary cross-entropy loss
            bce_loss = nn.functional.binary_cross_entropy(inputs_channel, targets_channel, reduction='none')

            # Apply class weights to the loss for the current channel
            weight_vector = targets_channel * self.weight[channel] + (1 - targets_channel) * self.weight[channel]
            weighted_bce_loss = weight_vector * bce_loss
